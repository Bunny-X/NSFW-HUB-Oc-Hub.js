...
async execute (client, message, args){
  ...
  const mw = [`**${message.author.tag}** está confundido.`, `**${message.author.tag}** está desorientado. o.o`, `**${message.author.tag}** se puso pacheco.`]
  const random = mw[Math.floor(Math.random() * mw.length)]


const action = await kakashi.confused();
